...
app.post("/api/rooms", (request, response) => {
  ...
  const new_room_no = Math.floor(Math.random() * 100000 + 1);
  const room = new Room({
    room_no: new_room_no,
    room_name: body.name,
    no_of_user: 1,
...
app.post("/api/completion", (request, response) => {
  const body = request.body;
  const new_room_no = Number(body.room_no);

  Room.find({ room_no: new_room_no }).then(result => {
    let room = result[0];
    ...
    let count = 0;
    room.completion.forEach(each_completion => {
      if (each_completion === 1) count++;
    });
    sendMoney(
      body.user_id,
  ...
  Room.find({ end_date: { $lt: date } }).then(result => {
    result.forEach(room => {
      ...
      user_ids.forEach(user_id => {
        if (completion[i] === 1) sendMoney(user_id, amount);
      });
